...
- 修复默认启动和 `--no-ipv4` 被错误地强制为 IPv4；显式命令行选项优先于 `BLREC_IPV4` 环境变量。
- 统一网络设置解析，正确识别 `False`、`0` 等关闭值；无效值在启动前明确报错且不回显原始内容。
- 默认恢复为系统自动选择 IPv4/IPv6。IPv6 线路不稳定时，可传 `--ipv4` 或设 `BLREC_IPV4=1` 保持旧版实际效果。
- 新增隔离进程回归测试；保留连接池、超时、代理、录制配置与 SEND_GIFT / SEND_GIFT_V2 行为。
- Docker 镜像：`ghcr.io/junpakugenso/blrec:2.0.4`；可使用 2.0.3 回退。网络设置需重启程序进程才生效，见 [维护约定](docs/stability-stage3.md)。

## 2.0.3（维护版）

- 网络连接池在当前运行环境中按需创建，避免导入阶段绑定到错误的事件循环。
- 同一事件循环保持连接复用；应用正常退出时统一关闭连接池，重启重新创建。
- 任务停止或销毁失败时保留连接供重试，避免误伤仍在运行的任务。
- 新增本机 HTTP、连接复用、请求取消、跨循环隔离与关闭重启测试；连接上限、超时、代理、配置和礼物协议不变。
- 独立使用内部 helper/Live 的脚本需显式清理连接池，见 [维护约定](docs/stability-stage2.md)。
- Docker 镜像：`ghcr.io/junpakugenso/blrec:2.0.3`；可使用 2.0.2 回退。

## 2.0.2（维护版）

- 配置使用独立快照串行保存，写入失败保留旧文件；取消操作不会提前释放写入锁。
- 启停失败后恢复状态，允许重试；录制启动失败时清理已启动的辅助资源和监听器。
- 修复 HLS 完成通知遗漏、满队列停止卡住和自然结束时的线程互相等待。
- 新增故障注入和完整停止链路回归测试；保留 SEND_GIFT / SEND_GIFT_V2 与现有配置格式。
- Docker 使用固定版本 `ghcr.io/junpakugenso/blrec:2.0.2`，启动、升级及回退见 [维护说明](docs/docker-maintenance.md)。
