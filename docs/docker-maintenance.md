...
There is no database or settings migration in 2.0.1. Avoid `latest` for deployments
where reproducibility matters; never use `docker volume rm` or `compose down -v`
as part of an upgrade.

## Release procedure

1. Increment `src/blrec/__init__.py` from 2.0.1 to 2.0.2, then 2.0.3, only when
